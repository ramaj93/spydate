...
internal static class JavaSugar
{
    ...
    private static (CStmt Try, int RemovedAfter)? Rewrite(JTry attempt, int any, List<CStmt> copy, List<CStmt> items, int index)
    {
        ...
        CStmt result = catches.Count == 0 && stripped is JTry { Finally: null } inner
