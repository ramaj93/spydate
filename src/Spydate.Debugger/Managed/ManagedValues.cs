...
internal static class ManagedValues
{
    ...
    internal static List<ManagedValue> Locals(ICorDebugILFrame frame)
        => Slots(frame, static (ICorDebugILFrame f, uint i, out IntPtr v) => f.GetLocalVariable(i, out v));

    /// <summary>Every argument in a frame. For an instance method, slot zero is <c>this</c>.</summary>
    internal static List<ManagedValue> Arguments(ICorDebugILFrame frame)
        => Slots(frame, static (ICorDebugILFrame f, uint i, out IntPtr v) => f.GetArgument(i, out v));

    private delegate int Slot(ICorDebugILFrame frame, uint index, out IntPtr value);

    private static List<ManagedValue> Slots(ICorDebugILFrame frame, Slot get)
    {
    ...
    private static ManagedValue Read(int index, IntPtr value)
    {
        int kind = Com.Borrow<ICorDebugValue, int?>(value, v => v.GetKind(out int k) == 0 ? k : null) ?? 0;
        string name = Name(kind);

        return new ManagedValue(index, name, Text(value, kind) ?? "not available");
    }

    /// <summary>What a value reads as, or null when the runtime would not produce it.</summary>
    private static string? Text(IntPtr value, int kind)
    {
        if (kind == ElementString)
        {
            return Str(value);
        }

        if (kind is ElementClass or ElementObject or ElementSzArray or ElementArray)
        {
            return Com.Borrow<ICorDebugReferenceValue, string>(value, reference =>
            {
                if (reference.IsNull(out int isNull) < 0)
                {
                    return null;
                }

                // A reference is followed no further than this. What it points at is a graph, and
                // walking one into an agent's context is a different feature with a budget.
                return isNull != 0 ? "null" : reference.GetValue(out ulong address) == 0 ? $"0x{address:X}" : "an object";
            });
        }
    ...
    private static string? Str(IntPtr value)
    {
        return Com.Borrow<ICorDebugReferenceValue, string>(value, reference =>
        {
            if (reference.IsNull(out int isNull) < 0)
            ...
            if (reference.Dereference(out IntPtr pointed) < 0 || pointed == IntPtr.Zero)
            {
                return null;
            }

            return Com.Owned<ICorDebugStringValue, string>(pointed, text =>
            {
